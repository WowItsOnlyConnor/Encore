...
private:
    SettingsOld() {}
    rapidjson::Value vectorToJsonArray(
        const std::vector<int> &vec, rapidjson::Document::AllocatorType &allocator
    ) {
    ...
    void ensureValuesExist() {
        ...
        if (!settings["controllerbinds"].HasMember("overdrive"))
            settings["controllerbinds"].AddMember(
                "overdrive", rapidjson::Value(), allocator
            );
        if (!settings["controllerbinds"].HasMember("4k_direction"))
            settings["controllerbinds"].AddMember(
                "4k_direction", rapidjson::kArrayType, allocator
            );
        if (!settings["controllerbinds"].HasMember("5k_direction"))
            settings["controllerbinds"].AddMember(
                "5k_direction", rapidjson::kArrayType, allocator
            );
        if (!settings["controllerbinds"].HasMember("overdrive"))
            settings["controllerbinds"].AddMember(
                "overdrive", rapidjson::Value(), allocator
        ...
        if (!settings["controllerbinds"].HasMember("pause_direction"))
            settings["controllerbinds"].AddMember(
                "pause_direction", rapidjson::Value(), allocator
            );
...
public:
    static SettingsOld &getInstance() {
        static SettingsOld instance; // This is the single instance
        return instance;
    }
    ...
    std::vector<int> defaultController4K { GLFW_GAMEPAD_BUTTON_DPAD_LEFT,
                                           GLFW_GAMEPAD_BUTTON_DPAD_RIGHT,
                                           GLFW_GAMEPAD_BUTTON_X,
                                           GLFW_GAMEPAD_BUTTON_B };
    std::vector<int> defaultController5K { GLFW_GAMEPAD_BUTTON_DPAD_LEFT,
                                           GLFW_GAMEPAD_BUTTON_DPAD_RIGHT,
                                           GLFW_GAMEPAD_BUTTON_X,
    ...
    std::vector<float> defaultTrackSpeedOptions = { 0.5f, 0.75f, 1.0f, 1.25f,
                                                    1.5f, 1.75f, 2.0f };
    ...
    void writeDefaultSettings(std::filesystem::path settingsFile, bool migrate = false) {
        ...
        settings["volume"].AddMember(
            "main", rapidjson::Value(defaultMainVolume), allocator
        );
        settings["volume"].AddMember(
            "player", rapidjson::Value(defaultPlayerVolume), allocator
        );
        settings["volume"].AddMember(
            "band", rapidjson::Value(defaultBandVolume), allocator
        );
        settings["volume"].AddMember("sfx", rapidjson::Value(defaultSFXVolume), allocator);
        settings["volume"].AddMember(
            "miss", rapidjson::Value(defaultMissVolume), allocator
        );
        settings["volume"].AddMember(
            "menu", rapidjson::Value(defaultMenuVolume), allocator
        );

        settings.AddMember(
            "keybinds", rapidjson::Value(rapidjson::kObjectType), allocator
        );
        ...
        settings["keybinds"].AddMember(
            "strumUp", rapidjson::Value(defaultKeybindStrumUp), allocator
        );
        settings["keybinds"].AddMember(
            "strumDown", rapidjson::Value(defaultKeybindStrumDown), allocator
        );
        settings["keybinds"].AddMember(
            "overdrive", rapidjson::Value(defaultKeybindOverdrive), allocator
        );
        settings["keybinds"].AddMember(
            "overdriveAlt", rapidjson::Value(defaultKeybindOverdriveAlt), allocator
        );
        settings["keybinds"].AddMember(
            "pause", rapidjson::Value(defaultKeybindPause), allocator
        );
        ...
        settings.AddMember(
            "controllerbinds", rapidjson::Value(rapidjson::kObjectType), allocator
        );
        settings["controllerbinds"].AddMember(
            "type", rapidjson::Value().SetInt(defaultControllerType), allocator
        );
        settings["controllerbinds"].AddMember("4k", arrayController4K, allocator);
        settings["controllerbinds"].AddMember(
            "4k_direction", arrayController4KDirection, allocator
        );
        settings["controllerbinds"].AddMember("5k", arrayController5K, allocator);
        settings["controllerbinds"].AddMember(
            "5k_direction", arrayController5KDirection, allocator
        );
        settings["controllerbinds"].AddMember(
            "overdrive", rapidjson::Value().SetInt(defaultControllerOverdrive), allocator
        );
        settings["controllerbinds"].AddMember(
            "overdrive_direction",
            rapidjson::Value().SetInt(defaultControllerOverdriveAxisDirection),
            allocator
        );
        settings["controllerbinds"].AddMember(
            "pause", rapidjson::Value().SetInt(defaultControllerPause), allocator
        );
        settings["controllerbinds"].AddMember(
            "pause_direction",
            rapidjson::Value().SetInt(defaultControllerPauseAxisDirection),
            allocator
        );
        ...
        for (std::filesystem::path &path : defaultSongPaths)
            arraySongDirs.PushBack(
                rapidjson::Value().SetString((const char *)(path.c_str()), allocator),
                allocator
            );
    ...
    void loadSettings(std::filesystem::path settingsFile) {
        ...
        if (std::filesystem::exists(settingsFile)) {
            ...
            if (settings.IsObject()) {
                ...
                if (settings.HasMember("keybinds") && settings["keybinds"].IsObject()) {
                    ...
                    if (settings["keybinds"].HasMember("4kAlt")
                        && settings["keybinds"]["4kAlt"].IsArray()) {
                    ...
                    if (settings["keybinds"].HasMember("5kAlt")
                        && settings["keybinds"]["5kAlt"].IsArray()) {
                    ...
                    if (settings["keybinds"].HasMember("strumUp")
                        && settings["keybinds"]["strumUp"].IsInt()) {
                    ...
                    if (settings["keybinds"].HasMember("strumDown")
                        && settings["keybinds"]["strumDown"].IsInt()) {
                    ...
                    if (settings["keybinds"].HasMember("overdrive")
                        && settings["keybinds"]["overdrive"].IsInt()) {
                    ...
                    if (settings["keybinds"].HasMember("overdriveAlt")
                        && settings["keybinds"]["overdriveAlt"].IsInt()) {
                        keybindOverdriveAlt =
                            settings["keybinds"]["overdriveAlt"].GetInt();
                        prevOverdriveAlt = keybindOverdriveAlt;
                    } else {
                        keybindsOverdriveAltError = true;
                    }
                    if (settings["keybinds"].HasMember("pause")
                        && settings["keybinds"]["pause"].IsInt()) {
                ...
                if (settings.HasMember("controllerbinds")
                    && settings["controllerbinds"].IsObject()) {
                    if (settings["controllerbinds"].HasMember("type")
                        && settings["controllerbinds"]["type"].IsInt()) {
                    ...
                    if (settings["controllerbinds"].HasMember("4k")
                        && settings["controllerbinds"]["4k"].IsArray()) {
                    ...
                    if (settings["controllerbinds"].HasMember("4k_direction")
                        && settings["controllerbinds"]["4k_direction"].IsArray()) {
                        const rapidjson::Value &arr =
                            settings["controllerbinds"]["4k_direction"];
                        if (arr.Size() == 4) {
                            for (int i = 0; i < 4; i++) {
                                if (arr[i].IsInt()) {
                    ...
                    if (settings["controllerbinds"].HasMember("5k")
                        && settings["controllerbinds"]["5k"].IsArray()) {
                    ...
                    if (settings["controllerbinds"].HasMember("5k_direction")
                        && settings["controllerbinds"]["5k_direction"].IsArray()) {
                        const rapidjson::Value &arr =
                            settings["controllerbinds"]["5k_direction"];
                        if (arr.Size() == 5) {
                            for (int i = 0; i < 5; i++) {
                                if (arr[i].IsInt()) {
                    ...
                    if (settings["controllerbinds"].HasMember("overdrive")
                        && settings["controllerbinds"]["overdrive"].IsInt()) {
                        controllerOverdrive =
                            settings["controllerbinds"]["overdrive"].GetInt();
                    ...
                    if (settings["controllerbinds"].HasMember("overdrive_direction")
                        && settings["controllerbinds"]["overdrive_direction"].IsInt()) {
                        controllerOverdriveAxisDirection =
                            settings["controllerbinds"]["overdrive_direction"].GetInt();
                    ...
                    if (settings["controllerbinds"].HasMember("pause")
                        && settings["controllerbinds"]["pause"].IsInt()) {
                    ...
                    if (settings["controllerbinds"].HasMember("pause_direction")
                        && settings["controllerbinds"]["pause_direction"].IsInt()) {
                        controllerPauseAxisDirection =
                            settings["controllerbinds"]["pause_direction"].GetInt();
                ...
                if (settings.HasMember("trackSpeedOptions")
                    && settings["trackSpeedOptions"].IsArray()) {
                    trackSpeedOptions = {};
                ...
                if (settings.HasMember("missHighwayColor")
                    && settings["missHighwayColor"].IsBool()) {
                ...
                if (settings.HasMember("songDirectories")
                    && settings["songDirectories"].IsArray()) {
        ...
        if (VolumeError) {
            ...
            settings.AddMember(
                "volume", rapidjson::Value(rapidjson::kObjectType), allocator
            );
            settings["volume"].AddMember(
                "main", rapidjson::Value(defaultMainVolume), allocator
            );
            ...
            settings["volume"].AddMember(
                "sfx", rapidjson::Value(defaultSFXVolume), allocator
            );
            settings["volume"].AddMember(
                "miss", rapidjson::Value(defaultMissVolume), allocator
            );
        ...
        if (controllerError) {
            ...
            settings["controllerbinds"].AddMember(
                "4k_direction", array4KDirection, allocator
            );
            settings["controllerbinds"].AddMember("5k", array5K, allocator);
            settings["controllerbinds"].AddMember(
                "5k_direction", array4KDirection, allocator
            );
            settings["controllerbinds"].AddMember(
                "overdrive", rapidjson::Value(defaultControllerOverdrive), allocator
            );
            settings["controllerbinds"].AddMember(
                "overdrive_direction",
                rapidjson::Value(defaultControllerOverdriveAxisDirection),
                allocator
            );
            settings["controllerbinds"].AddMember(
                "pause", rapidjson::Value(defaultControllerPause), allocator
            );
            settings["controllerbinds"].AddMember(
                "pause_direction",
                rapidjson::Value(defaultControllerPauseAxisDirection),
                allocator
            );
        }
        if (controllerTypeError) {
            ...
            settings["controllerbinds"].AddMember(
                "type", rapidjson::Value(defaultControllerType), allocator
            );
        }
        ...
        if (trackSpeedOptionsError) {
            ...
            rapidjson::Value arrayTrackSpeedOptions(rapidjson::kArrayType);
            for (float &speed : defaultTrackSpeedOptions)
                arrayTrackSpeedOptions.PushBack(
                    rapidjson::Value().SetFloat(speed), allocator
                );
            settings.AddMember("trackSpeedOptions", arrayTrackSpeedOptions, allocator);
        }
        ...
        if (MenuVolumeError || MissVolumeError || keybindsStrumDownError
            || keybindsStrumUpError || SFXVolumeError || BandVolumeError
            || PlayerVolumeError || VolumeError || MainVolumeError || fullscreenError
            || songDirectoryError || highwayLengthError || mirrorError || MissHighwayError
            || keybindsError || keybinds4KError || keybinds5KError || keybinds4KAltError
            || keybinds5KAltError || keybindsOverdriveError || keybindsOverdriveAltError
            || keybindsPauseError || controllerError || controllerTypeError
            || controller4KError || controller5KError || controllerOverdriveError
            || controller4KDirectionError || controller5KDirectionError
            || controllerOverdriveDirectionError || controllerPauseError
            || controllerPauseDirectionError || avError || inputError || trackSpeedError
            || trackSpeedOptionsError) {
            ensureValuesExist();
            saveSettings(settingsFile);
        }
    }
    void migrateSettings(
        std::filesystem::path keybindsFile, std::filesystem::path settingsFile
    ) {
    ...
    const void saveSettings(std::filesystem::path settingsFile) {
        ...
        rapidjson::Value::MemberIterator MainVolumeMember =
            settings["volume"].FindMember("main");
        MainVolumeMember->value.SetFloat(MainVolume);
        rapidjson::Value::MemberIterator PlayerVolumeMember =
            settings["volume"].FindMember("player");
        PlayerVolumeMember->value.SetFloat(PlayerVolume);
        rapidjson::Value::MemberIterator BandVolumeMember =
            settings["volume"].FindMember("band");
        BandVolumeMember->value.SetFloat(BandVolume);
        rapidjson::Value::MemberIterator SfxVolumeMember =
            settings["volume"].FindMember("sfx");
        SfxVolumeMember->value.SetFloat(SFXVolume);
        rapidjson::Value::MemberIterator MissVolumeMember =
            settings["volume"].FindMember("miss");
        MissVolumeMember->value.SetFloat(MissVolume);
        rapidjson::Value::MemberIterator MenuVolumeMember =
            settings["volume"].FindMember("menu");
        MenuVolumeMember->value.SetFloat(MenuVolume);

        rapidjson::Value::MemberIterator fullscreenMember =
            settings.FindMember("fullscreen");
        ...
        rapidjson::Value::MemberIterator trackSpeedMember =
            settings.FindMember("trackSpeed");
        ...
        rapidjson::Value::MemberIterator inputOffsetMember =
            settings.FindMember("inputOffset");
        inputOffsetMember->value.SetInt(inputOffsetMS);
        rapidjson::Value::MemberIterator missHighwayColorMember =
            settings.FindMember("missHighwayColor");
        ...
        rapidjson::Value::MemberIterator songDirMember =
            settings.FindMember("songDirectories");
        songDirMember->value.Clear();
        for (std::filesystem::path &path : songPaths)
            songDirMember->value.PushBack(
                rapidjson::Value().SetString(path.string(), allocator), allocator
            );
        rapidjson::Value::MemberIterator keybinds4KMember =
            settings["keybinds"].FindMember("4k");
        keybinds4KMember->value.Clear();
        for (int &key : keybinds4K)
            keybinds4KMember->value.PushBack(rapidjson::Value().SetInt(key), allocator);
        rapidjson::Value::MemberIterator keybinds5KMember =
            settings["keybinds"].FindMember("5k");
        keybinds5KMember->value.Clear();
        for (int &key : keybinds5K)
            keybinds5KMember->value.PushBack(rapidjson::Value().SetInt(key), allocator);
        rapidjson::Value::MemberIterator keybinds4KAltMember =
            settings["keybinds"].FindMember("4kAlt");
        ...
        rapidjson::Value::MemberIterator keybinds5KAltMember =
            settings["keybinds"].FindMember("5kAlt");
        ...
        rapidjson::Value::MemberIterator strumUpKeyMember =
            settings["keybinds"].FindMember("strumUp");
        strumUpKeyMember->value.SetInt(keybindStrumUp);
        rapidjson::Value::MemberIterator strumDownKeyMember =
            settings["keybinds"].FindMember("strumDown");
        strumDownKeyMember->value.SetInt(keybindStrumDown);
        rapidjson::Value::MemberIterator overdriveKeyMember =
            settings["keybinds"].FindMember("overdrive");
        overdriveKeyMember->value.SetInt(keybindOverdrive);
        rapidjson::Value::MemberIterator overdriveKeyAltMember =
            settings["keybinds"].FindMember("overdriveAlt");
        overdriveKeyAltMember->value.SetInt(keybindOverdriveAlt);
        rapidjson::Value::MemberIterator pauseKeyMember =
            settings["keybinds"].FindMember("pause");
        pauseKeyMember->value.SetInt(keybindPause);
        rapidjson::Value::MemberIterator controller4KMember =
            settings["controllerbinds"].FindMember("4k");
        ...
        rapidjson::Value::MemberIterator controller5KMember =
            settings["controllerbinds"].FindMember("5k");
        ...
        rapidjson::Value::MemberIterator controller4KDirMember =
            settings["controllerbinds"].FindMember("4k_direction");
        controller4KDirMember->value.Clear();
        for (int &key : controller4KAxisDirection)
            controller4KDirMember->value.PushBack(
                rapidjson::Value().SetInt(key), allocator
            );
        rapidjson::Value::MemberIterator controller5KDirMember =
            settings["controllerbinds"].FindMember("5k_direction");
        controller5KDirMember->value.Clear();
        for (int &key : controller5KAxisDirection)
            controller5KDirMember->value.PushBack(
                rapidjson::Value().SetInt(key), allocator
            );
        rapidjson::Value::MemberIterator overdriveControllerMember =
            settings["controllerbinds"].FindMember("overdrive");
        overdriveControllerMember->value.SetInt(controllerOverdrive);
        rapidjson::Value::MemberIterator overdriveControllerDirMember =
            settings["controllerbinds"].FindMember("overdrive_direction");
        overdriveControllerDirMember->value.SetInt(controllerOverdriveAxisDirection);
        rapidjson::Value::MemberIterator pauseControllerMember =
            settings["controllerbinds"].FindMember("pause");
        pauseControllerMember->value.SetInt(controllerPause);
        rapidjson::Value::MemberIterator controllerTypeMember =
            settings["controllerbinds"].FindMember("type");
